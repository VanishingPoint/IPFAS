...
#define SECONDS_PER_PERIOD 5//to demo at daid
#define PERIODS_TO_WARN 2  //CHANGED FOR DEMO
...
#define TOP_ANCHOR_ADDRESS 8
#define BOTTOM_ANCHOR_ADDRESS 7
...
//Check  alert status from bluetooth service here (no longer necessary)
...
//Test!! -> The if and the while This did fix the services, but then main code body did not run...
//if (central) { //just the if made the code repeat like crazy!
  
//  while (central.connected()) { //Just the while fixes the services but the code does not run!
...
//Halfway though the period, read the first anchor and switch anchors
//if (i == (CYCLES_PER_SECOND * SECONDS_PER_PERIOD)/2) {
...
//At the end of the period, read the second anchor and switch anchors
//readAnchorVal(anchorOn); 
//switchAnchorTo(!anchorOn);
...
void calculateHorizontalWarn (void) {
  ...
  Serial.println(bottomAnchorDist);
  ...
  if(currentAngleRad>boundryRad) {
   LatestAnglePeriod = 1;
   Serial.print("Horizontal Warn Period");
   if (angleWarnPeriods<PERIODS_TO_WARN)
     angleWarnPeriods++;
  } else {
   LatestAnglePeriod = 0;
   Serial.print("Horizontal Distance OK");
